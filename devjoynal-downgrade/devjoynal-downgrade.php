...
function downgrade_load_textdomain() {
	load_plugin_textdomain( 'devjoynal-downgrade', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
}
...
function downgrade_register_menu() {
	add_submenu_page(
		'options-general.php',
		__( 'DevJoynal Downgrade', 'devjoynal-downgrade' ),
		__( 'DevJoynal Downgrade', 'devjoynal-downgrade' ),
		'update_core',
		'devjoynal-downgrade',
...
function downgrade_sanitize_url( $value ) {
	...
	if ( ! $url || ! wp_http_validate_url( $url ) ) {
		add_settings_error( 'wpdg_messages', 'invalid_url', __( 'Enter a valid HTTP or HTTPS ZIP URL.', 'devjoynal-downgrade' ), 'error' );
...
function downgrade_plugin_action_links( $links ) {
	$settings_link = sprintf(
		'<a href="%s">%s</a>',
		esc_url( admin_url( 'options-general.php?page=devjoynal-downgrade' ) ),
		esc_html__( 'Settings', 'devjoynal-downgrade' )
	);
...
function downgrade_handle_reset() {
	...
	if ( ! current_user_can( 'update_core' ) || ! check_admin_referer( 'downgrade_reset_pin_action', 'downgrade_reset_pin_nonce' ) ) {
		wp_die( esc_html__( 'You are not allowed to reset the Downgrade settings.', 'devjoynal-downgrade' ) );
	}
	...
	<div class="wrap">
		...
		<div class="downgrade-grid">
			<div class="downgrade-card">
				<h2><?php esc_html_e( 'Core version control', 'devjoynal-downgrade' ); ?></h2>
				<p class="downgrade-status"><span class="downgrade-dot <?php echo $target ? 'active' : 'inactive'; ?>"></span><?php echo $target ? esc_html__( 'Version pin is active', 'devjoynal-downgrade' ) : esc_html__( 'Version pin is inactive', 'devjoynal-downgrade' ); ?></p>
				<div class="downgrade-warning"><strong><?php esc_html_e( 'Before you continue:', 'devjoynal-downgrade' ); ?></strong> <?php esc_html_e( 'Create and verify a complete files-and-database backup. Test on staging first. A Core change may affect plugins, themes, database data, and server requirements.', 'devjoynal-downgrade' ); ?></div>
				...
				<form method="post" class="downgrade-actions">
					...
					<button class="button" type="submit" name="downgrade_reset_pin" value="1"><?php esc_html_e( 'Reset all Downgrade settings', 'devjoynal-downgrade' ); ?></button>
					<a class="button button-secondary" href="<?php echo esc_url( admin_url( 'update-core.php' ) ); ?>"><?php esc_html_e( 'Open Update Core', 'devjoynal-downgrade' ); ?></a>
				</form>
			</div>
