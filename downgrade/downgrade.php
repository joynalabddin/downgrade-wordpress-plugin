...
function downgrade_load_plugin_textdomain() {
$loaded = load_plugin_textdomain( 'downgrade', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
// Translation loading is intentionally silent; WordPress continues in English if no catalog is available.
}
add_action('plugins_loaded', 'downgrade_load_plugin_textdomain');
...
function register_downgrade_settings() {
	...
	register_setting( 'wpdg-settings-group', 'wpdg_specific_version_name', array('sanitize_callback' => 'downgrade_sanitize_version') );
...
function downgrade_sanitize_version($userstring)
// Sicherstellen, dass eine plausible Versionsnummer eingegeben wurde (andernfalls Sicherheitsrisiko). Ein 'sanitize_text_field' oder sowas würde ggf. auch reichen, aber so prüfen wir genauer, dass der Input wirklich zum pattern passt. 
{
  if (!preg_match("/^[-+]?[0-9]*[.]?[0-9]?[.]?[0-9]+$/", $userstring) AND $userstring !== ''){
        add_settings_error('prefix_messages', 'wpdg_message', __('Version number looks strange.', 'downgrade'), 'error');
        return;
    } else {
        return $userstring;
    }    
...
add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), 'downgrade_action_links' );
function downgrade_action_links( $links ) {
   $links[] = '<a href="'. esc_url( get_admin_url(null, 'options-general.php?page=downgrade') ) .'">Settings</a>';
   return $links;
}
...
<h3><?php _e('Downgrade', 'downgrade'); ?>: <?php if (get_option('wpdg_specific_version_name')) { ?><span style="color: green;"><?php _e('Active', 'downgrade'); ?> (<?php _e('WP', 'downgrade'); ?> <?php echo esc_html(get_option('wpdg_specific_version_name')); ?> <?php _e('is set as target version', 'downgrade'); ?>)</span><?php } else {; ?><span style="color: red;"><?php _e('Inactive', 'downgrade'); ?></span><?php }; ?></h3>

   <p><?php _e('WARNING! You are using this plugin entirely at your own risk! DO MAKE SURE you have a current backup of both your files and database, since a manual version change is deeply affecting your WP installation!', 'downgrade'); ?></p>

<h3><?php _e('Which WordPress version would you like to up-/downgrade to?', 'downgrade'); ?></h3>
...
<form method="post" action="options.php">
    ...
    <table class="form-table">
      ...
      <col width="180px">
        <tr valign="top">
        <th scope="row"><?php _e('WordPress Target Version', 'downgrade'); ?>:</th>
        ...
        <td><?php esc_html_e('Exact version number from', 'downgrade'); ?> <a href="<?php echo esc_url($release_link); ?>" target="_blank"><?php esc_html_e('WP Releases', 'downgrade'); ?></a>, <?php esc_html_e('for example "4.9.8". Leave empty to disable.', 'downgrade'); ?></td>
        </tr>
        
        <tr valign="top">
        <th scope="row"><?php esc_html_e('Current WP Version', 'downgrade'); ?>:</th>
        <td><?php echo esc_html($wp_version); ?></td>
        <td></td>
        </tr>
        
        <tr valign="top">
        <th scope="row"><?php esc_html_e('Language Detected', 'downgrade'); ?>:</th>
    ...
    ?>
        <tr valign="top">
        <td><input type="checkbox" id="myCheck" onclick="wpdgshowhide()" name="wpdg_edit_download_url"  <?php if (get_option('wpdg_edit_download_url')){ echo 'checked';} ?>> <?php esc_html_e('edit download URL', 'downgrade'); ?> </td>
...
if (version_compare($wp_version, get_option('wpdg_specific_version_name') ) == 0 ) { ?>
  ...
  <?php wp_kses(_e('<strong>All fine!</strong> You are currently on your desired release. And it will stay like that. <br>If you ever want to <strong>reinstall</strong> ', 'downgrade'), $allowed_tags); ?> <?php echo esc_html(get_option('wpdg_specific_version_name')).', you have to switch to another version and come back. If you want to return to the regular update channel, you need to empty the version number above and go to '; ?> <a href="<?php echo esc_url(get_admin_url( null, '/update-core.php' )) ;?>"><?php esc_html_e('Update Core', 'downgrade'); ?></a>.
  </div>
<?php } else { ?>
<div style="border: 2px solid orange; padding: 5px;">
  <p><strong><?php esc_html_e('In order to perform the upgrade/downgrade to WP', 'downgrade'); ?> <?php echo esc_html(get_option('wpdg_specific_version_name')); ?> <?php esc_html_e('please go to', 'downgrade'); ?> <a href="<?php echo esc_url(get_admin_url( null, '/update-core.php' )) ;?>"><?php esc_html_e('Update Core', 'downgrade'); ?></a>. </strong></p>
  <p><a href="<?php echo esc_url(get_admin_url( null, '/update-core.php' )) ;?>" class="button"><?php esc_html_e('Up-/Downgrade Core', 'downgrade'); ?></a></p>
</div>
<?php } ?>

<?php if (wpdg_urlcheck(wpdg_get_url(get_option('wpdg_specific_version_name'))) == false){ ?>
<div style="border: 2px solid red; padding: 5px;">
<span style="color: red;"> <?php esc_html_e('Attention! The target version does not seem to exist!', 'downgrade'); ?> </span><br>
<span style="color: red;"> URL: <?php echo esc_url(wpdg_get_url(get_option('wpdg_specific_version_name')));  ?></span><br>
<span style="color: red;"> <?php esc_html_e('The update could fail. Are you sure that the version number is correct? You can also manually edit the download URL if needed.', 'downgrade'); echo " <strong>". esc_html(get_option('wpdg_specific_version_name')); ?> </strong></span><br>
</div>
<?php }
